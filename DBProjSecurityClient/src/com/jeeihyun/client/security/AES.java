package com.jeeihyun.client.security;
public class AES {
    private int[][] state =new int[4][4];
    private int[][] keyary = new int[4][4];
    private int[][] plaintext = new int[4][4];
    private int[][] prekey = new int[11][16];
    ...
    private void shiftAry(int row){
        ...
        for (int i = 1; i < 4; i++) state[row][i-1]=state[row][i];
        state[row][3]=temp;
    }
    ...
    public String Function(String msg,String keys){
        String key=keys;
        String result = "";
        String padmsg = "";
        String ms = msg;
        while(key.length()%32 != 0) key = key + 0;
        while(ms.length()%16 != 0) ms = ms + 0;

        for (int j = 0; j < ms.length(); j++) padmsg += String.format("%02X", (int) ms.charAt(j));

        result = encryptAES(padmsg,key);

        return result;
    }
    public String Functionbar(String msg,String keys){
        String key=keys;
        String padkey = "";
        String result = "";
